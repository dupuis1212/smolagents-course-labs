"""Module 8 smoke tests — reliable agents: a validated QuillReport + final_answer_checks.

Live call budget: ~4 real LLM runs (the carried-forward M7 bench = 2 runs, the M6 multi-turn
= 2 runs on one agent, the new M8 live run = 1), well under 10 LLM calls (only when
...
repo-root conftest.py), and the M8 checks are also asserted DIRECTLY as plain functions.

This file carries the Module 2/3/4/5/6/7 tests forward (the toolbox, the agent loop,
make_model(), the Monitor cost accessor, the sandbox policy, the callbacks/multi-turn and the
planning/instructions still work — note the carried-forward agent-loop tests now pass
final_answer_checks=[] to opt OUT of the new contract, since they assert pre-M8 mechanics) and
adds the Module 8 tests:
...
Run from the repo root: ``uv run pytest module-08/tests/``
"""
