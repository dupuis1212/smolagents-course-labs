...
Module 14 makes Quill OBSERVABLE and MEASURABLE. ``quill/telemetry.py`` adds ``instrument()`` —
``SmolagentsInstrumentor().instrument()`` (OpenTelemetry/OpenInference) called BEFORE the agent is
built, driven by ``QUILL_TELEMETRY`` (Langfuse/Phoenix; default ``none`` = no-op). ``quill/eval/``
adds the FROZEN eval harness: a ``golden_set.json`` + ``run_evals.py`` that runs Quill over the
golden set, scores each run with the ``judge_report`` LLM-as-judge (reusing the FROZEN
``QuillReport`` schema — M8), aggregates a Task Success Rate + cost/run (via ``Monitor``, M4), and
applies a regression gate. ``build_quill`` and ``QuillReport`` are UNCHANGED — telemetry and eval
live in their own modules, around the agent.

Later modules ADD to this package (never delete): ``team.py`` (M10), ``retriever.py`` (M12),
``telemetry.py`` + ``eval/`` (M14), and so on.
"""
from .agent import (
    CHART_REVIEW_PROMPT,
...
# M14: telemetry instrumentation (eval lives in the quill.eval subpackage, imported on demand).
from .telemetry import instrument, resolve_backend
