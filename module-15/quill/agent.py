...
**Module 10 change:** Quill stops being a one-person band and becomes a **manager**. The big
shift: the web tools (``WebSearchTool`` + ``VisitWebpageTool``) LEAVE the manager's own
``tools=`` list and move INTO a specialised sub-agent, the **``web_researcher``** (a
``ToolCallingAgent`` built in ``quill/team.py``). ``build_quill`` constructs that sub-agent and
passes it to the manager via ``managed_agents=[web_researcher]`` — the current multi-agent
mechanism (a sub-agent becomes callable through its ``name`` + ``description``; there is NO
``ManagedAgent``, removed in 1.26.0). Quill (the manager ``CodeAgent``) now plans and analyses
on its OWN clean memory and DELEGATES web research; the sub-agent runs its own ReAct loop,
visits pages, and returns only a short summary with source URLs — the manager never has to
digest raw pages. That is **context isolation**, the real reason for multi-agent (NOT "more
agents = better results"). The run still ends in a cited ``QuillReport`` (M8, unchanged).

``build_quill`` is EXTENDED (never broken) with ONE more keyword argument, defaulting so every
prior call site is unchanged:

- ``managed_agents: list | None = None`` — sub-agents to register on the manager. Omit it and
  Quill builds its DEFAULT team (just the ``web_researcher``); pass ``[]`` for a SOLO manager
  with no team (the pre-M10 single-agent shape, minus the web tools); pass your own list to
  customise. Each entry must be an agent carrying a ``name`` + ``description``.

The manager STAYS ``executor_type="local"`` here ON PURPOSE (T10.7): a remote executor +
``managed_agents`` raises ``Exception("Managed agents are not yet supported with remote code
execution.")`` (``create_python_executor`` refuses it — Approach 1 ships no secrets into the
sandbox, so a sub-agent could not authenticate its LLM). Running the whole team inside a sandbox
is Approach 2, the capstone (Module 15). ``make_model`` (the researcher uses it too),
``QuillReport``, ``save_chart`` and the import lock are all untouched.

**Module 11 change:** Quill gains EYES. Two things, both off the prior mandatory path:

- **Chart self-review (the headline, ``review_charts``).** Until now Quill wrote a matplotlib
  chart, saved it with ``save_chart``, and moved on WITHOUT ever looking at the pixels — it
  trusted its own code. Now, after a run, ``review_charts(report, model=...)`` loads each
  ``QuillReport.chart_paths`` PNG as a PIL image and asks a **VLM** to critique it through
  ``run(images=[...])`` ("is this chart misleading? truncated axis? unreadable legend?"). The
  verdict is appended to the report's existing ``caveats`` — NO new ``QuillReport`` field (the M8
  schema is frozen). The critical detail (objective #1 of the module): the image goes via
  ``images=`` (vision content the model LOOKS AT), NOT ``additional_args=`` (a named Python
  variable the sandbox operates on). A model only *sees* an image you pass through ``images=`` —
  and only if it is a VLM. The ``[vision]`` extra (helium+selenium) is NOT what enables this;
  image input needs only a VLM, no extra at all.

- **The OPTIONAL ``vision_browser`` (``browse=True``).** ``build_quill(browse=True)`` ADDS the
  ``vision_browser`` ``CodeAgent`` (``quill/team.py``) to the default team, so the manager can
  delegate JS/chart-heavy pages a text scraper cannot read. It drives helium/Chrome and reads
  screenshots via the ``save_screenshot`` step_callback. It STAYS ``executor_type="local"`` (helium
  needs local Chrome; remote + managed raises the M10 exception — "# M15: Approach 2"). Default
  ``browse=False`` so every prior call site is unchanged and no Chrome is ever opened by surprise.

``build_quill`` is EXTENDED (never broken) with ONE more keyword argument, ``browse: bool =
False``. ``make_model`` (the VLM comes through it too), ``QuillReport``, ``save_chart`` and the
import lock are all untouched.

**Module 12 change:** Quill stops INVENTING the meaning of ambiguous columns and starts
RETRIEVING it. ``build_quill`` now ADDS a ``RetrieverTool`` (``quill/retriever.py``) to the
manager's own ``tools=`` list, next to the data tools and ``save_chart``. It is a BM25 retriever
over a docs corpus (`data/corpus/*.md` — a data dictionary plus revenue/segmentation rules): when
Quill meets a column like ``net_rev`` it calls ``retriever("net_rev definition")`` in its
sandboxed code, reads the passage, and CITES it in ``QuillReport.sources`` instead of guessing.
Because the retriever is a *tool the agent chooses to call*, this is AGENTIC RAG — the agent can
reformulate the query and retrieve again (HyDE/self-query EMERGE from the code-as-action loop, they
are not smolagents APIs), and on a question that touches no ambiguous column it may never call the
retriever at all (a fixed pipeline would always retrieve, once, before generating).

The retriever does its BM25 scoring in normal Python INSIDE its own ``forward`` (it is a ``Tool``,
run by the framework around the sandbox), so the manager's FROZEN least-privilege import lock is
UNTOUCHED — ``rank_bm25`` is NOT added to ``additional_authorized_imports``: the agent's generated
code only ever calls ``retriever("...")``, it never imports the BM25 lib (the tool does, lazily, in
its ``setup()``). ``make_model`` (the retriever calls no LLM — BM25 is purely lexical),
``QuillReport`` (the corpus citations reuse the existing ``sources`` field — NO new field),
``save_chart`` and the import lock are all untouched. The corpus ``Source``s and the web ``Source``s
(M10) cohabit in the same ``sources`` list.

``build_quill`` is EXTENDED (never broken) with ONE more keyword argument, ``retrieve: bool =
True``: by default Quill carries the ``RetrieverTool`` (the M12 headline). Pass ``retrieve=False``
for the pre-M12 shape (no knowledge base — the bench baseline, or a deployment with no docs corpus).

**Module 15 change (the capstone — hardening, by ADDITION; ``build_quill`` is NOT moved).** Two
hardening levers land here, both pure additions (every prior call site is unchanged):

- **Bounded step caps** were already set inside ``build_quill`` (manager ``max_steps=8``,
  ``web_researcher`` ``max_steps=10``) — an agent that does not converge raises
  ``AgentMaxStepsError`` (M8) instead of looping forever. The capstone simply states this is the
  reliability ceiling and does not raise it.
- **Idempotence** of ``save_chart`` paths. ``run_quill_report(agent, csv, question)`` sets a
  deterministic run signature (``runtime.run_signature(question, dataset)``) BEFORE the run and
  clears it after, so an un-named ``save_chart`` writes ``outputs/quill-<sig>.png`` — a re-run of the
  same (question, dataset) OVERWRITES the same file rather than littering ``chart-<timestamp>.png``.
  ``save_chart``'s FROZEN signature is untouched (the auto-name change is internal, env-driven).

The Approach-2 orchestration (run the WHOLE team inside a sandbox) lives in the NEW
``quill/runtime.py``, which CALLS ``build_quill`` — it never moves it (06 §2). ``build_quill`` stays
the single construction owner; ``runtime.py`` only assembles + sandboxes + hardens around it.
...
``final_answer_checks`` in M8, ``extra_tools`` in M9, ``managed_agents`` in M10, ``browse`` in
M11, ``retrieve`` in M12. The optional ``model=None`` is what lets tests inject an offline fake
model and what lets ``make_model()`` own the real default. M15 adds NO new ``build_quill``
parameter — it hardens AROUND the existing one.
"""
...
from .team import build_vision_browser, build_web_researcher
from .tools import clear_run_signature, load_dataset, profile_dataframe, save_chart, set_run_signature
...
QUILL_INSTRUCTIONS = (
    ...
    "When a column or metric meaning is AMBIGUOUS (e.g. net_rev, churn_flag, region_code), do "
    "NOT invent it — call retriever('<column> definition') to look it up in the data dictionary, "
    "use the retrieved definition, and record that document as a Source so you can cite it [n]. "
    "If the passages do not answer you, rewrite the query and retrieve again.\n"
    ...
    "Prefer the fewest steps that answer the question correctly — retrieve only when you "
    "actually need a definition, not on every step."
)
...
# The critique prompt the VLM reads when re-reading a chart (Module 11). It names the exact
# failure modes a chart can hide from its author — a truncated/zoomed Y-axis that exaggerates a
# trend, an overplotted mess, an off-canvas legend, a misleading dual axis — and asks for a SHORT
# verdict. Short on purpose: the verdict lands in QuillReport.caveats, not a wall of text.
CHART_REVIEW_PROMPT = (
    ...
    "Answer in ONE or TWO sentences: state the single biggest problem and a concrete fix, or say "
    "the chart looks fine. Be concise — your answer becomes a caveat on the report."
)
...
# Sentinel so the TWO intents of `managed_agents=` stay distinguishable (M10):
#   omitted                 -> Quill's DEFAULT team (build the web_researcher and register it)
#   managed_agents=[]       -> a SOLO manager, no team (pre-M10 single-agent shape, web tools gone)
#   managed_agents=[a, b]   -> the caller's own sub-agents (each must carry name + description)
# We reserve _DEFAULT_TEAM for "build Quill's team" so an explicit [] is honoured as "no team".
_DEFAULT_TEAM = object()
...
def build_quill(
    ...
    *,
    ...
    managed_agents: list | None = _DEFAULT_TEAM,  # type: ignore[assignment]
    browse: bool = False,
    retrieve: bool = True,
) -> CodeAgent:
    ...
    The toolbox (M10: the web tools are GONE from here — they live in the ``web_researcher``):
    - ``load_dataset`` / ``profile_dataframe`` (``@tool`` functions) and ``save_chart``
      (a ``Tool`` subclass — note ``save_chart()``: it is instantiated here).
    - NO ``WebSearchTool`` / ``VisitWebpageTool`` on the manager anymore. Web access now belongs
      to the ``web_researcher`` sub-agent (``quill/team.py``), registered via ``managed_agents``.
      The manager reaches the web by DELEGATING — that is the context isolation this module is for.
    ...
    Args:
        ...
        extra_tools (M9): a list of additional ``Tool`` objects APPENDED to Quill's frozen local
            toolbox (the local data tools stay first and unchanged). This is the seam for tools
            from OUTSIDE this repo — MCP tools (``ToolCollection.from_mcp`` / ``MCPClient``), a Hub
            tool (``load_tool`` / ``Tool.from_hub``), a LangChain tool (``Tool.from_langchain``).
            Default ``None`` = no extras. ``run_with_mcp`` passes the MCP server's tools here. You
            can also add a tool at runtime via ``agent.tools[t.name] = t``. NOTE: MCP tools run
            OUTSIDE the sandbox (server-side / in the stdio subprocess); only the Python Quill
            writes to CALL them runs in the sandbox — see ``quill/tools/mcp.py`` for the gate.
        managed_agents (M10): the sub-agents to register on the manager via
            ``CodeAgent(managed_agents=...)``. Omit it to get Quill's DEFAULT team — just the
            ``web_researcher`` (``build_web_researcher`` in ``quill/team.py``); pass ``[]`` for a
            SOLO manager with no team (the pre-M10 single-agent shape, minus the web tools); pass
            your own list to customise. Each entry must be an agent carrying ``name`` +
            ``description`` (that is what makes it callable by the manager — there is NO
            ``ManagedAgent``). When the team is built by default, the researcher's model is chosen
            the SAME way as the manager: ``model`` if you passed one (so tests get one fake model
            for the WHOLE team), otherwise ``make_model(role="researcher")``.
        browse (M11, OPTIONAL): when ``True`` AND the DEFAULT team is in use (``managed_agents``
            omitted), ADD the ``vision_browser`` ``CodeAgent`` (``build_vision_browser`` in
            ``quill/team.py``) to the team, so the manager can delegate JS/chart-heavy pages a
            text scraper cannot read. Default ``False`` so no Chrome is ever opened by surprise and
            every prior call site is unchanged. The vision_browser drives helium/Chrome and reads
            screenshots via the ``save_screenshot`` step_callback; for a REAL browse its model must
            be a VLM (point ``QUILL_MODEL_ID`` at one). It stays ``executor_type="local"`` (helium
            needs local Chrome; remote + managed raises — Approach 2 is M15). ``browse`` is ignored
            when you pass your OWN ``managed_agents`` list (you are in full control then) — add a
            ``build_vision_browser(...)`` to that list yourself.
        retrieve (M12): when ``True`` (the default — the M12 headline), ADD a ``RetrieverTool``
            (``quill/retriever.py``) to the manager's OWN toolbox, so Quill can look up an ambiguous
            column's meaning in the docs corpus (``data/corpus/*.md``) and CITE it instead of
            guessing — agentic RAG (the agent decides when to call it and may reformulate). The
            retriever does its BM25 scoring inside its own ``forward`` (a ``Tool``), so the FROZEN
            least-privilege import lock is untouched (``rank_bm25`` is NOT added to
            ``additional_authorized_imports``). Pass ``retrieve=False`` for the pre-M12 shape (no
            knowledge base) — the bench baseline, or a deployment with no corpus.
    ...
    - The executor STAYS ``local`` (resolve_executor reads QUILL_EXECUTOR, but a remote value +
      ``managed_agents`` raises — see the guard below). Approach 2 (whole team in a sandbox) is M15.
    ...
    # M10: Quill's frozen local DATA toolbox FIRST, then any ecosystem tools (MCP / Hub /
    # LangChain) the caller supplies via extra_tools. The web tools (WebSearchTool /
    # VisitWebpageTool) are NO LONGER here — they moved into the web_researcher sub-agent so the
    # manager's memory stays clean (context isolation). Appending keeps the local tools' names/
    # order stable; the toolbox is a name-keyed dict, so distinct names (e.g. read_query) won't
    # clash. save_chart STAYS on the manager (it draws charts from the data it analyses).
    tools = [
        load_dataset,
        profile_dataframe,
        save_chart(),  # Tool subclass: instantiate it (setup() runs lazily on 1st call)
    ]
    # M12: give the manager a knowledge base it queries on demand (agentic RAG). RetrieverTool is a
    # Tool subclass like save_chart — instantiate it; its BM25 index builds lazily in setup() on the
    # first retriever("...") call and is reused after. It does its scoring in its OWN forward (run by
    # the framework), so the agent's generated code never imports rank_bm25 — the FROZEN
    # least-privilege import lock stays untouched (no rank_bm25 in additional_authorized_imports).
    if retrieve:
        tools.append(RetrieverTool())
    if extra_tools:
        tools.extend(extra_tools)

    # M10: resolve the `managed_agents` intent — omitted -> Quill's DEFAULT team (just the
    # web_researcher); an explicit list (including []) is honoured verbatim. The researcher's
    # model is chosen the SAME way as the manager's: the caller's `model` if given (so a single
    # fake model powers the WHOLE team in offline tests), else make_model(role="researcher").
    # `name` + `description` on the sub-agent are what make it callable here — NEVER ManagedAgent.
    # M11: with browse=True (and only the DEFAULT team), ADD the vision_browser CodeAgent. It
    # shares the injected `model` too (one fake model drives the whole team offline). It stays
    # executor_type="local": helium needs local Chrome, and remote + managed_agents would raise
    # the M10 exception.  # M15: Approach 2 (whole team in a sandbox) lifts the local constraint.
    if managed_agents is _DEFAULT_TEAM:
        ...
        resolved_managed_agents = managed_agents
    ...
    agent = CodeAgent(
        ...
        # M10: register the research team. A sub-agent is callable because it carries name +
        # description (build_web_researcher sets both); it is NEVER a ManagedAgent (removed in
        # 1.26.0). The manager (this CodeAgent) reaches the team by writing `web_researcher("...")`
        # in its sandboxed code — smolagents exposes each managed agent as a function there. The
        # executor MUST stay local for this to be legal: a remote executor + managed_agents raises
        # `Exception("Managed agents are not yet supported with remote code execution.")` because
        # Approach 1 ships no secrets into the sandbox (the sub-agent could not auth its LLM). So
        # running this team needs executor_type="local" (here) OR Approach 2 (whole team inside the
        # sandbox), which is the capstone, Module 15.
        managed_agents=resolved_managed_agents,
        max_steps=8,
    )
...
def _load_chart_image(path: str):
    """Load a saved chart PNG as a PIL image, ready to hand to ``run(images=[...])`` (M11).

    The image is what the VLM LOOKS AT. PIL is imported lazily (Pillow ships with the data stack;
    no extra needed for image INPUT — that is the whole point of the module). A missing/unreadable
    file raises ``ValueError`` with a fix-it message instead of crashing the review silently.
    """
    import os
    ...
    if not os.path.exists(path):
        raise ValueError(
            f"No chart to review at {path!r}. review_charts re-reads the PNGs save_chart "
            "produced (QuillReport.chart_paths); the path must exist on disk."
        )
    # .copy() detaches the image from the file handle so it survives after the file is closed.
...
def review_charts(
    ...
    *,
    model: Model | None = None,
    prompt: str = CHART_REVIEW_PROMPT,
) -> QuillReport:
    """Have a VLM RE-READ the charts in ``report`` and append its verdict to ``caveats`` (M11).

    The module headline: Quill stops trusting its own matplotlib blind. After a run produced a
    ``QuillReport`` with ``chart_paths``, this loads each PNG as a PIL image and asks a **VLM** to
    critique it — through ``run(images=[pil_image])``. The verdict is APPENDED to the report's
    existing ``caveats`` (e.g. *"Y-axis starts at 80, which exaggerates the trend — recommend
    starting at 0"*). No new ``QuillReport`` field: the M8 schema is FROZEN, and ``caveats`` is
    exactly the place for an honest limitation.

    **The channel is ``images=``, NOT ``additional_args=`` (objective #1 of the module).** The
    image must be VISION CONTENT the model LOOKS AT, so it goes through ``run(images=[...])``,
    which smolagents injects as a ``TaskStep.task_images`` block in the message stream. Passing it
    via ``additional_args={"chart": pil}`` would only put a Python variable in the sandbox — the
    model would NOT see the pixels (it could only write code that touches the object). And the
    model must be a **VLM**: image input is a property of the MODEL, not of any extra — the
    ``smolagents[vision]`` extra (helium+selenium) is a BROWSER, it does NOT enable image input.

    The reviewer model comes from ``make_model`` (the M4 frozen contract — point ``QUILL_MODEL_ID``
    at a VLM such as ``Qwen/Qwen2-VL-72B-Instruct`` or ``gpt-4o`` via litellm). Pass your own
    ``model`` to run offline (a fake model in tests). We build a BARE ``CodeAgent`` (no data tools)
    just to read the image and return a one-line verdict — a separate, cheap call, not part of the
    analysis loop (the production note: you re-read at the END, not every step — a VLM call is
    expensive).

    Args:
        report: the ``QuillReport`` whose ``chart_paths`` to re-read. Returned UNCHANGED if it has
            no charts (nothing to review).
        model: a VLM ``smolagents.Model``; if ``None``, ``make_model(role="reviewer")`` is used.
        prompt: the critique instruction (default ``CHART_REVIEW_PROMPT``).

    Returns:
        The SAME ``report`` object, with one caveat appended per chart reviewed (``"Chart review
        (<path>): <verdict>"``). Mutating + returning it keeps the call-site ergonomic
        (``report = review_charts(report)``).
    """
    if not report.chart_paths:
        return report

    # A bare reviewer agent: no data tools, it only looks at the image and answers. It is NOT the
    # analyst manager — re-reading a chart is a separate, end-of-run sanity check. We pass
    # final_answer_checks=[] (we want a free-form verdict string, not a QuillReport) and
    # managed_agents=[] (no team — this agent does not browse or delegate).
    reviewer = CodeAgent(
        tools=[],
        model=model or make_model(role="reviewer"),
        managed_agents=[],
        max_steps=2,
    )

    for path in report.chart_paths:
        image = _load_chart_image(path)
        # THE channel: images= (vision content the model looks at). reset=True so each chart is
        # judged on its own. A fresh agent.run per chart keeps the verdicts independent.
...
def build_retrieval_task(csv_path: str, question: str) -> str:
    """Phrase a task that nudges Quill to RETRIEVE + CITE an ambiguous column's meaning (M12).

    Like ``build_report_task`` but it explicitly tells Quill that the column meanings are NOT
    obvious from the names and that a ``retriever`` tool exists over the project's data dictionary.
    Quill is free to decide WHEN to call it (agentic RAG) — but the question is chosen to touch an
    ambiguous column (``net_rev`` / ``churn_flag``), so a grounded answer must look the definition
    up and cite the corpus doc as a ``Source`` in ``QuillReport.sources``. The corpus ``url``/
    ``title`` come back embedded in the retriever's output (see ``quill/retriever.py``), so the
    agent can build the ``Source`` directly from a retrieved passage.

    Used by the CLI's ``--retrieve`` demo and the live test; offline tests script the retriever
    call + report code directly.
    """
    return (
        f"The dataset is at {csv_path}. Question: {question}\n"
        "Steps:\n"
        "1. profile_dataframe to learn the schema. The column names are NOT self-explanatory.\n"
        "2. For any column or metric whose meaning matters to the answer (e.g. net_rev, "
        "churn_flag, region_code), do NOT guess — call retriever('<column> definition') to look it "
        "up in the data dictionary, and KEEP the returned passage's title and url for citation. If "
        "the passages do not answer you, rewrite the query and retrieve again.\n"
        "3. Write pandas to compute the answer using the retrieved definitions.\n"
        "4. Draw a matplotlib chart that backs the answer and call save_chart to save it.\n"
        "5. Build a report with the QuillReport class (already available — do NOT import it):\n"
        "     rep = QuillReport(question=<the question>, findings=[...], chart_paths=[<saved "
        "path>], sources=[Source(url=<corpus url>, title=<corpus title>)], caveats=[...])\n"
        "   Cite each retrieved definition as [1], [2], ... in the matching finding, with the "
        "Source order matching the numbers.\n"
        "6. Call final_answer(rep). A report with no saved chart will be REJECTED — fix it and "
        "answer again."
    )
...
def run_quill_report(
    ...
    *,
    ...
    """Run Quill for a validated ``QuillReport`` with M15 idempotence wired around it.

    The hardened single-run helper the capstone uses (CLI + sandbox entrypoint). It:

    1. derives a deterministic run signature from (question, dataset) and sets it
       (``set_run_signature``), so an un-named ``save_chart`` writes ``outputs/quill-<sig>.png`` — a
       re-run of the SAME question + dataset OVERWRITES the same file (idempotence, T12.14),
    2. ensures the ``outputs/`` dir exists idempotently (``exist_ok=True``),
    3. runs ``build_report_task`` on the agent (the team + the default ``final_answer_checks`` ->
       a cited ``QuillReport``, M8),
    4. ALWAYS clears the signature in a ``finally`` (resource management taught explicitly, 03 §3 —
       not a silent except), so a later run with no signature falls back to the timestamp auto-name.

    ``build_quill`` already bounds ``max_steps`` (manager 8, ``web_researcher`` 10) — the step-cap
    half of hardening; this helper adds the idempotence half. It does NOT change ``save_chart``'s
    FROZEN signature (the auto-name is env-driven and only applies when no filename is passed).

    Args:
        agent: a built Quill (``build_quill``) — tests inject a fake-model one.
        csv_path: the dataset the run analyses.
        question: the analysis question.
        return_full_result: forwarded to ``agent.run`` (a ``RunResult`` when ``True``).

    Returns:
        The run output (a ``QuillReport`` for a normal report run), or a ``RunResult`` when
        ``return_full_result=True``.
    """
    # Imported here so quill.agent does not import quill.runtime at module load (runtime imports
    # agent — avoid a cycle). run_signature is a pure function (sha256), no heavy deps.
    from .runtime import ensure_outputs_dir, run_signature

    ensure_outputs_dir("outputs")
    set_run_signature(run_signature(question, csv_path))
    try:
        return agent.run(
            build_report_task(csv_path, question), return_full_result=return_full_result
        )
    finally:
        # Always clear: the deterministic auto-name was for THIS run only (03 §3 resource mgmt).
        clear_run_signature()
...
def main() -> None:
    """CLI: ``uv run python -m quill.agent [--multi-turn] [--browse] [--review] <csv_path> [<question>]``.
    ...
    ``--browse`` (M11): add the OPTIONAL ``vision_browser`` sub-agent so the manager can delegate
    JS/chart-heavy pages it cannot scrape as text. Needs a local Chrome + the ``[vision]`` extra +
    a VLM. ``--review`` (M11, on by default for the single run): after the run, RE-READ each chart
    in the report with a VLM via ``review_charts`` (``run(images=[...])``) and append the verdict
    to ``QuillReport.caveats``. This is the "Quill checks the chart itself" path — it needs a VLM
    (point ``QUILL_MODEL_ID`` at one), no extra. Pass ``--no-review`` to skip it.
    """
    import sys
    ...
    browse = "--browse" in args
    review = "--no-review" not in args  # chart self-review is the M11 default for a single run
    ...
    # M11: browse=True adds the vision_browser to the team (helium/Chrome) — stays local.
    agent = build_quill(browse=browse)
    ...
    # M11: Quill RE-READS its own charts with a VLM (run(images=...)) as a sanity check, and the
    # verdict lands in QuillReport.caveats. This is a SEPARATE, end-of-run call (a VLM call is
    # expensive — you do not re-read every step). Needs a VLM model; no [vision] extra.
    output = result.output
    if review and isinstance(output, QuillReport) and output.chart_paths:
        print("\n===== CHART SELF-REVIEW (VLM re-reads the saved charts via run(images=...)) =====")
        output = review_charts(output)
