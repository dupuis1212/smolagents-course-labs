...
# The web entry points Quill can use. When the agent's generated code references one of these, the
# resulting report makes web-backed claims, so it must cite at least one source. The first two are
# the canonical smolagents tool names (`WebSearchTool().name`, `VisitWebpageTool().name`), used
# directly by the manager up to Module 9. As of Module 10 the web tools moved into the
# `web_researcher` sub-agent, so the manager reaches the web by DELEGATING — its code now says
# `web_researcher(...)` instead of `web_search(...)`. We ADD that name (an extension, not a schema
# change — the QuillReport fields and the 3-arg check signatures are FROZEN, 06 §2) so a delegated
# web run still trips the "did Quill go to the web?" heuristic and must carry a source.
WEB_TOOL_NAMES = ("web_search", "visit_webpage", "web_researcher")
