"""Module 9 smoke tests — tool interop: MCP, the Hub, and other ecosystems.

Live call budget: ~5 real LLM runs (the carried-forward M7 bench = 2 runs, the M6 multi-turn
= 2 runs on one agent, the M8 live report = 1; the NEW M9 live MCP run = 1), still under 10 LLM
calls (only when QUILL_LIVE_TESTS=1 and HF_TOKEN is set). The M9 live MCP test ALSO needs `uvx`
and a built `data/sales.db` — it skips cleanly if either is missing. Sandbox budget: 1-2 Docker
runs (marked `sandbox`, skipped if Docker is absent). Everything else runs OFFLINE with no
network, no token, and NO MCP connection: the MCP tests assert the helper SHAPES and import the
MCP entry points, but NEVER open a server (no subprocess) — connecting is a `live` test only.

Module 9 (NEW) — quill/tools/mcp.py + quill/agent.py + quill/scripts/* + quill/demos/*:
...
Run from the repo root: ``uv run pytest module-09/tests/``
"""
...
# `quill*` so this file always binds to module-08/quill.
...
# ======================================================================================
# Module 9 (NEW): MCP entry points exist and are imported the CORRECT way (T9.1, T9.7, 06 §6).
...
def test_build_quill_default_has_no_extra_tools_only_the_frozen_toolbox(fake_model):
    """Omitting extra_tools -> Quill's exact M2-M8 toolbox (no ecosystem tools added)."""
    agent = build_quill(model=fake_model(["final_answer('ok')"]))
    names = set(agent.tools)
    assert {"load_dataset", "profile_dataframe", "save_chart", "web_search",
            "visit_webpage", "final_answer"} <= names
...
def test_web_backed_report_without_a_source_is_rejected_then_corrected(fake_model):
    """The web-source check end to end (06 §6 step 3 + 5): step 1 calls web_search (the evidence
    lands in memory), step 2 returns a report with NO source (rejected by
    check_has_source_for_web_claims), step 3 adds a Source and is accepted. The web call is in an
    EARLIER step than the answer, so it is already in agent.memory.steps when the check reads it
    (the validator runs before the CURRENT step is appended). The web tool is stubbed (OFFLINE)."""
    step1 = "hits = web_search('saas category growth 2025')\nprint(str(hits)[:30])"
    step2 = (
    ...
    # Stub the web tool so no network is hit (offline).
    agent.tools["web_search"].forward = lambda query, **kw: f"STUB RESULTS for {query}"
