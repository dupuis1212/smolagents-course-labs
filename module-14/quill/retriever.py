...
**Pushable by construction (06 §2, research-02 §8) — hardened in Module 13.** This is
deliberate so Module 13 can ``agent.push_to_hub(...)`` the WHOLE Quill (tools included)
without a rewrite. smolagents' ``Tool.to_dict`` serialises a ``Tool`` subclass by reading ONLY
its own class source with a static AST validator (``validate_tool_attributes``), so EVERY rule
below is load-bearing for the push to succeed:

- ``__init__`` takes NO argument beyond ``self`` (the corpus comes from the CLASS attribute
  ``corpus_dir``, read in ``setup()``). Constructor args are "not traceable" and fail validation.
- ``corpus_dir``/``k`` are **literal** class attributes (``"data/corpus"`` / ``5``), NOT
  references to module-level constants. A non-literal class attribute trips the validator's
  "Complex attributes should be defined in __init__" rule. They still EQUAL ``DEFAULT_CORPUS_DIR``
  / ``DEFAULT_K`` (kept below so the lab/tests reference the same default).
- Every import lives INSIDE a method (``from rank_bm25 import BM25Okapi`` in ``setup``), and
  ``setup`` is SELF-CONTAINED — it inlines the corpus walk rather than calling the module-level
  ``load_corpus`` helper, because the saved tool carries only its own method bodies (a call to an
  outside ``load_corpus`` would fail with "Name 'load_corpus' is undefined"). ``load_corpus`` stays
  for non-tool callers and tests, but the TOOL no longer depends on it.

The official smolagents RAG example passes ``docs`` to ``__init__`` and uses
``langchain_community.BM25Retriever``; we deviate on both — no constructor args (pushable) and no
LangChain dependency (``rank-bm25`` directly).
...
class RetrieverTool(Tool):
    ...
    # M13: written as LITERALS (not `DEFAULT_CORPUS_DIR`/`DEFAULT_K`), because Tool.to_dict's static
    # validator rejects a non-literal class attribute ("Complex attributes should be defined in
    # __init__"). They still equal the module constants below — same value, now serialisable.
    ...
    def setup(self) -> None:
        ...
        M13 pushable rule: the corpus walk is INLINED here (it does NOT call the module-level
        ``load_corpus``) so the saved tool — which carries only this method's body — has no
        undefined outside reference. ``load_corpus`` (below) keeps the SAME logic for non-tool
        callers and tests; this inline copy is what gets serialised on ``agent.push_to_hub``.
        """
        import pathlib

        from rank_bm25 import BM25Okapi

        # Read corpus_dir/*.md into {title, url, text} dicts (the load_corpus logic, inlined so the
        # pushed tool is self-contained). Sorted by filename for a deterministic citation order.
        documents: list[dict] = []
        for path in sorted(pathlib.Path(self.corpus_dir).glob("*.md")):
            text = path.read_text(encoding="utf-8")
            first_line = text.lstrip().splitlines()[0] if text.strip() else path.stem
            title = first_line.lstrip("# ").strip() if first_line.startswith("#") else path.stem
            documents.append({"title": title, "url": path.as_posix(), "text": text})
